...
const getApartmentForRentMogi = async () => {
    let data = []
    for( let i=3001; i<= 4500; i++ ) {
        request('https://mogi.vn/thue-can-ho?cp=' + i, async (error, response, html) => {
                    ...
                    request(url, async (error, response, html) => {
                        if(!error && response.statusCode == 200) {
                            ...
                            await $('.main-info').each(async (index, el) => { 
                                    ...
                                    fs.writeFileSync('apartmentForRentMogi3.json', JSON.stringify(data, null, 10)); 
...
getApartmentForRentMogi()

module.exports =  getApartmentForRentMogi; 
