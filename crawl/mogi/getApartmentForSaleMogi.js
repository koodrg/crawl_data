...
const getApartmentForSaleMogi = async () => {
    let data = []
    for( let i=401; i<= 905; i++ ) {
        request('https://mogi.vn/mua-can-ho?cp=' + i, async (error, response, html) => {
                    ...
                    request(url, async (error, response, html) => {
                        if(!error && response.statusCode == 200) {
                            ...
                            await $('.main-info').each(async (index, el) => { 
                                ...
                                fs.writeFileSync('apartmentForSaleMogi_1.json', JSON.stringify(data, null, 10)); 
