...
const getHomeForSale = async () => {
    let data = []
    //1132
    for( let i=500; i<= 1000; i++ ) {
        request('https://bds123.vn/ban-nha.html?page=' + i, async (error, response, html) => {
                    ...
                    request(url, async  (error, response, html) => {
                        if(!error && response.statusCode == 200) {
                            ...
                            await $('.leftCol article.the-post-detail').each(async (index, el) => { 
                                ...
                                fs.writeFileSync('homeForSale.json', JSON.stringify(data, null, 10)); 
