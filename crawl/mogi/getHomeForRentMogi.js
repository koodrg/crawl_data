...
const getApartmentForSaleMogi = async () => {
    let data = []
    for( let i=3101; i<= 4000; i++ ) {
        request('https://mogi.vn/thue-nha?cp=' + i, async (error, response, html) => {
                    ...
                    request(url, async (error, response, html) => {
                        if(!error && response.statusCode == 200) {
                            ...
                            await $('.main-info').each(async (index, el) => { 
                                    ...
                                    fs.writeFileSync('homeForRentMogi_1.json', JSON.stringify(data, null, 10)); 
