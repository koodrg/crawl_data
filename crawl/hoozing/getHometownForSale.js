...
const getApartmentForRent = async  () => {
    ...
    for( let i=1; i< 20;i++ ) {
        ...
        request('https://hoozing.com/mua-ban/nha-pho-ho-chi-minh?view_mode=list&page=' + i, async (error, response, html) => {
            ...
            await $('.apartment-item').each((index, el) => { // lặp từng phần tử có class là job__list-item
                ...
                request(url, (error, response, html) => {
                    if(!error && response.statusCode == 200) {
                        ...
                        fs.writeFileSync('hometownForSale.json', JSON.stringify(data, null , 12)); 
                    }
                    else {
                        console.log(error);
                    }
