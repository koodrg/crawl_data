...
const getApartmentForRent = async () => {
    ...
    for( let i=501; i<= 1000; i++ ) {
        request('https://bds123.vn/cho-thue-can-ho-chung-cu.html?page=' + i, async (error, response, html) => {
                    ...
                    request(url, async  (error, response, html) => {
                        if(!error && response.statusCode == 200) {
                            ...
                            await $('.leftCol article.the-post-detail').each(async (index, el) => { 
                                ...
                                fs.writeFileSync('apartmentForRent_bds123.json', JSON.stringify(data, null, 10)); 
