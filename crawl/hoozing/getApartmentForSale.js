...
const getApartmentForSale = async  () => {
    ...
    for( let i=1; i< 128;i++ ) {
        ...
        request('https://hoozing.com/mua-ban/can-ho-ho-chi-minh?view_mode=list&page=' + i, async (error, response, html) => {
            ...
            await $('.apartment-item').each((index, el) => { // lặp từng phần tử có class là job__list-item
                ...
                request(url, (error, response, html) => {
                    if(!error && response.statusCode == 200) {
                        ...
                        fs.writeFileSync('apartmentForSale.json', JSON.stringify(data, null, 10)); 
                    }
                    else {
                        console.log(error);
                    }
