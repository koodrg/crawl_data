...
const getLandForRentMogi = async () => {
    let data = []
    for( let i=1; i<= 150; i++ ) {
        request('https://mogi.vn/thue-nha-xuong-kho-bai-dat?cp=' + i, async (error, response, html) => {
            if(!error && response.statusCode == 200) {
                const $ = cheerio.load(html); // load HTML
                await $('ul.props li').each((index, el) => { 
                    let url = $(el).find('.prop-info a.link-overlay').attr('href').toString();
                    let category = 'Đất mặt bằng cho thuê'
                    //console.log(url)
                    ...
                    request(url, async (error, response, html) => {
                        if(!error && response.statusCode == 200) {
                            ...
                            await $('.main-info').each(async (index, el) => { 
                                ...
                                fs.writeFileSync('landForRentMogi_1.json', JSON.stringify(data, null, 10)); 
...
module.exports =  getLandForRentMogi; 
