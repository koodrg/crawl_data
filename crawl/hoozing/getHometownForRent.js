...
const getApartmentForRent = async  () => {
    ...
    for( let i=1; i< 7;i++ ) {
        ...
        request('https://hoozing.com/cho-thue/nha-pho-ho-chi-minh?view_mode=list&page=' + i, async (error, response, html) => {
            ...
            await $('.apartment-item').each((index, el) => { // lặp từng phần tử có class là job__list-item
                ...
                request(url, (error, response, html) => {
                    if(!error && response.statusCode == 200) {
                        ...
                        fs.writeFileSync('hometownForRent.json', JSON.stringify(data, null, 12)); 
                    }
                    else {
                        console.log(error);
                    }
